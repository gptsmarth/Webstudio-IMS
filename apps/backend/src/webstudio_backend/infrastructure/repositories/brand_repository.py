...
class BrandRepository(SqlAlchemyRepository[Brand]):
    ...
    async def create(self, name: str, *, is_active: bool = True) -> Brand:
        ...
        return await self.add(Brand(name=normalized, is_active=is_active))

    async def update_name(self, brand: Brand, name: str) -> Brand:
        normalized = normalize_required_name(name)
