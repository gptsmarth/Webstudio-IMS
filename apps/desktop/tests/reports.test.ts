...
describe('reportBuilder', () => {
  it('maps inventory status filters', () => {
    const available = buildReportQueryParams('inventory', {
      ...DEFAULT_REPORT_FILTERS,
      inventoryStatus: 'available',
    }, 1, 50, 'created_at', 'desc');
    expect(available.status).toBe('available');
    expect(available.is_archived).toBe(false);

    const archived = buildReportQueryParams('inventory', {
      ...DEFAULT_REPORT_FILTERS,
  ...
  it('maps inventory created date preset to query range', () => {
    const params = buildReportQueryParams('inventory', {
      ...DEFAULT_REPORT_FILTERS,
      datePreset: 'month',
    }, 1, 50, null, 'desc');
  ...
  it('maps tally outcome filters to notification types', () => {
    ...
    const params = buildReportQueryParams('tally', {
      ...DEFAULT_REPORT_FILTERS,
  ...
  it('requires preview workflow — export params mirror active filters', () => {
    const params = buildReportQueryParams('sales', {
      ...DEFAULT_REPORT_FILTERS,
