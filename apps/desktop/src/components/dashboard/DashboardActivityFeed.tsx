    ...
    <DashboardWidget title={title} subtitle={subtitle}>
      {loading ? (
        ...
        <DashboardEmptyState
          ...
          description="Activity will appear here as operations occur."
        />
      ) : (
        <ul className="dash-feed">
          {filtered.slice(0, 6).map((item) => (
            <li key={item.id} className="dash-feed__item">
              ...
              <span className="dash-feed__meta">
                {item.actor_display_name ?? 'System'} · {formatRelativeTime(item.created_at)}
              </span>
