    ...
    <ModalPortal>
      <div className="stg-restore-overlay" role="presentation" onClick={onClose}>
        ...
        <header className="stg-recovery-wizard__header">
          <div>
            <h2><Wand2 size={18} aria-hidden /> Recovery wizard</h2>
            <p className="stg-muted">Guide administrators through validation, recovery, and verification.</p>
          </div>
        ...
        <div className="stg-recovery-wizard__body">
          {step === 1 && center && (
            <>
              <div className={`stg-recovery-banner stg-recovery-status--${center.system_health === 'healthy' ? 'healthy' : 'warning'}`}>
              ...
              <button type="button" className="btn btn-primary btn-sm" disabled={busy} onClick={() => void runValidation()}>
          ...
          {step === 4 && (
            <>
              <p>
                If you restored the entire database, restart the WEBSTUDIO backend and desktop client
                so connections reload with the recovered data.
              </p>
          ...
          {step === 5 && (
            <>
              ...
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => void loadCenter()}>
                Refresh status
              </button>
              {center && (
                <p className={`stg-backup-verify stg-backup-verify--${center.recovery_readiness === 'ready' ? 'success' : 'warning'}`}>
        ...
        <footer className="stg-recovery-wizard__footer">
          ...
          {step < 5 ? (
            ...
            <button type="button" className="btn btn-primary btn-sm" onClick={() => void finish()}>
