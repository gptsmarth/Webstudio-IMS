...
export function RestoreWizard({
  ...
  const canProceedFromValidate =
    (validation?.valid && validation.restore_allowed !== false) ?? false;
          ...
          <header className="stg-restore-wizard__header">
            <div>
              ...
              <p className="stg-muted">
                Enterprise recovery workflow with validation and rollback.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="stg-restore-wizard__body">
                      ...
                      <button
                        ...
                        className={[
                          ...
                          selectedFilename === item.filename
                            ? 'stg-restore-select-item--active'
                            : '',
                        ]
                          .filter(Boolean)
                          .join(' ')}
            ...
            {step === 2 && validation && (
              <section>
                ...
                <div className="stg-restore-checks">
                  <CheckRow
                    label="Checksum"
                    ok={validation.checksum_valid}
                    warn={!validation.checksum_valid}
                  />
                  ...
                  <CheckRow
                    label="Compatibility"
                    ok={validation.compatibility === 'compatible'}
                    warn={validation.compatibility === 'warning'}
                  />
                </div>
                ...
                <div className="stg-readonly-grid">
                  ...
                  <Readonly
                    ...
                    value={validation.migration_required ? 'Yes' : 'No'}
                  />
                </div>
                {validation.warnings.map((warning) => (
                  <p key={warning} className="stg-backup-warning">
                    {warning}
                  </p>
                ))}
                {validation.errors.map((entry) => (
                  <p key={entry} className="stg-backup-error">
                    {entry}
                  </p>
                ))}
              </section>
                  ...
                  <>
                    <div className="stg-readonly-grid">
                      ...
                      <Readonly
                        label="Database size"
                        value={
                          preview.database_size_bytes
                            ? formatBytes(preview.database_size_bytes)
                            : '—'
                        }
                      />
                      <Readonly
                        label="Compressed size"
                        value={formatBytes(preview.compressed_size_bytes ?? 0)}
                      />
                      <Readonly
                        label="Checksum status"
                        value={preview.checksum_valid ? 'Valid' : 'Unverified'}
                      />
                      <Readonly
                        label="Schema compatibility"
                        value={preview.schema_compatibility ?? '—'}
                      />
                      <Readonly
                        label="Affected areas"
                        value={preview.affected_areas.join(', ') || '—'}
                      />
                    </div>
            ...
            {step === 5 && (
              <section className="stg-restore-progress">
                ...
                <p className="stg-muted">
                  Creating emergency backup, applying restore scope, and verifying…
                </p>
              </section>
            )}
            ...
            {step === 6 && result && (
              <section>
                <h3>Verification</h3>
                <div
                  className={`stg-backup-verify stg-backup-verify--${result.verification_status}`}
                >
                ...
                {result.restart_required && (
                  <div className="stg-warning">
                    ...
                    <span>
                      Backend restart is required for a full database restore to take full effect.
                    </span>
                  </div>
                )}
                {result.rollback_available &&
                  (result.rollback_recommended || result.emergency_backup_filename) && (
                    <button
                      type="button"
                      className="btn btn-ghost btn-sm"
                      disabled={busy}
