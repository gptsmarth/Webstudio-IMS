...
import { Archive, ArchiveRestore, Eye, MapPin, Pencil, QrCode, ShoppingBag, Tag } from 'lucide-react';
...
export function InventoryRowActionsMenu({
  ...
  return createPortal(
    ...
    >
      <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('view', item)}>
        <Eye size={14} aria-hidden /> View
      </button>
      {canWrite && (
        <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('edit', item)}>
          <Pencil size={14} aria-hidden /> Edit
        </button>
      )}
      {canWrite && !item.is_archived && item.status !== 'sold' && (
        <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('transfer', item)}>
          <MapPin size={14} aria-hidden /> Transfer
        </button>
      )}
      {canSell && item.status !== 'sold' && !item.is_archived && (
        <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('markSold', item)}>
          <ShoppingBag size={14} aria-hidden /> Mark sold
        </button>
      )}
      {canWrite && !item.is_archived && (
        <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('archive', item)}>
          <Archive size={14} aria-hidden /> Archive
        </button>
      )}
      {canWrite && item.is_archived && (
        <button type="button" className="inv-row-menu__item" role="menuitem" onClick={() => onAction('restore', item)}>
          <ArchiveRestore size={14} aria-hidden /> Restore
        </button>
      )}
      <button type="button" className="inv-row-menu__item inv-row-menu__item--disabled" disabled title="Coming soon">
